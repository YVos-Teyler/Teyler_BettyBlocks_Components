(() => ({
    ...
    jsx: (() => {
      ...
      if (isDev && !(hasLink || hasExternalLink)) {
        linkedContent = (
          <Tag
            className={[classes.content, options.hoverActive ? classes.hover : ''].join(' ')}
    ...
    styles: (B) => (t) => {
      ...
      return {
        ...
        hover: {
            ...
            '&:hover': { 
                scale: ({options: {hoverScale}}) => 1 + (hoverScale/100),
                color: ({ options: { hoverTextColor, type } }) => {
                    return hoverTextColor === '[Inherit]'
                    ? style.getFontColor(type)
                    : style.getColor(hoverTextColor);
                },
                backgroundColor: ({ options: { hoverBackgroundColor, type } }) => {
                    return hoverBackgroundColor === '[Inherit]'
                        ? style.getFontColor(type)
                        : style.getColor(hoverBackgroundColor);
                }
            }
        }
      };
