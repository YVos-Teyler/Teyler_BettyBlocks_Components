    ...
    jsx: (
        <span>
            {(() => {
                ...
                return (
                  <>
                  <B.GetAll modelId={options.model} filter={options.filter} skip={0} take={1}> 
                    {({ loading, error, data, refetch }) => {
                        if (isDev) { return <Tag class={classes.placeholder}>BigSum</Tag> }
                        ...
                        if (name) {
                          const batchSize = 1
                          const countBatches = Math.ceil(totalCount / batchSize);
                          console.log('# batches', countBatches);
                          let result = 0;
                          let i =0;
                          for (let i = 0; i < countBatches; i++) {
                            
                            result += <B.GetAll modelId={options.model} filter={options.filter} skip={i*batchSize} take={batchSize}> 
                              {({ batchloading, batcherror, batchdata, batchrefetch }) => {
                               
                                  const { kind, modelId, name } = B.getProperty(options.property);
                                  const { batchresults, batchtotalCount } = batchdata;
                                  console.log(i, 'batchresult', batchresults)
                                  const batchresult = batchresults.reduce((partialSum, item) => partialSum + item[name], 0)
                                  console.log(i, batchresult)
                                  return <span>{i}: {batchresult}</span>
                              }}
                            </B.GetAll>
                            ...
                            console.log(i, data)
                            result += data.reduce((partialSum, item) => partialSum + item[name], 0)
